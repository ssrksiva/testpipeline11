pipeline {
  ...
  stages {
    stage('Pull') {
      steps {
        git(url: 'https://github.com/ssrksiva/testpipeline11.git', branch: 'develop', poll: true)
      }
    }
    ...
    stage('Deliver for development') {
      ...
      steps {
        ...
        withCredentials(bindings: [usernamePassword(credentialsId: 'testgithubcred', usernameVariable: 'user', passwordVariable: 'pass')]) {
          ...
          sh 'git push https://${user}:${encodedPass}@github.com/${user}/testpipeline11.git HEAD:release -f'
        }
    ...
    stage('Deliver for feature') {
      ...
      steps {
        ...
        withCredentials(bindings: [usernamePassword(credentialsId: 'testgithubcred', usernameVariable: 'user', passwordVariable: 'pass')]) {
          ...
          sh 'git push https://${user}:${encodedPass}@github.com/${user}/testpipeline11.git HEAD:develop -f'
        }
    ...
    stage('Deliver for hotfix') {
      ...
      steps {
        ...
        withCredentials(bindings: [usernamePassword(credentialsId: 'testgithubcred', usernameVariable: 'user', passwordVariable: 'pass')]) {
          ...
          sh 'git push https://${user}:${encodedPass}@github.com/${user}/testpipeline11.git HEAD:master -f'
        }
